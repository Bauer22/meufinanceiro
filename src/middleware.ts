...
export async function middleware(req: NextRequest) {
  ...
  if (!password) {
    // Em desenvolvimento local sem senha configurada, deixa passar.
    if (process.env.NODE_ENV === "development") return NextResponse.next();
    const msg = "Defina APP_PASSWORD (e AUTH_SECRET) nas variáveis de ambiente para liberar o acesso.";
    return isApi
      ? NextResponse.json({ error: msg }, { status: 503 })
      : new NextResponse(msg, { status: 503, headers: { "content-type": "text/plain; charset=utf-8" } });
  }

  const ok = await verifyToken(authSecret(), req.cookies.get(COOKIE)?.value);
